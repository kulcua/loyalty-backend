...
require("./modules/admin.suggestion-box/module.js");
require("./modules/admin.chat/module.js");

if (!window.OpenLoyaltyConfig.debug) {
  if (!window.console) window.console = {};
  ["log", "debug", "warn", "info"].map(
    (method) => (console[method] = function () {})
  );
}
...
angular
  ...
  .config(function (
      ...
      if (extractedData.customers) {
        for (let i in data.customers) {
          if (extractedData.customers[i].customerId) {
            extractedData.customers[i].id =
              extractedData.customers[i].customerId;
          }
        }
      }
    ...
    jwtInterceptorProvider.tokenGetter = [
      ...
      "config",
      "$state",
      function (AuthService, jwtHelper, config, $state) {
        if (
          config.url.indexOf("login_check") !== -1 ||
          config.url.indexOf("token/refresh") !== -1
        ) {
          return null;
        }
        ...
        if (
          idToken &&
          jwtHelper.isTokenExpired(idToken) &&
          AuthService.isRememberMe()
        ) {
          return AuthService.getRefreshToken().then(
            (res) => {
  ...
  .run([
    ...
    function (
      ...
      $rootScope.pendingRequests = _.isNumber($rootScope.pendingRequests)
      ...
      $templateCache.put(
        ...
        '<input type="text" name="{{name}}" ng-disabled="$filterRow.disabled" ng-model="params.filter()[name]" class="input-filter form-control" placeholder="{{getFilterPlaceholderValue(filter, name)}}" ng-model-options="{debounce: 1500}" /> '
      );
      $templateCache.put(
        "ng-table/filters/number.html",
        '<input type="number" name="{{name}}" ng-disabled="$filterRow.disabled" ng-model="params.filter()[name]" class="input-filter form-control" placeholder="{{getFilterPlaceholderValue(filter, name)}}" ng-model-options="{debounce: 1500}" /> '
      );
    },
  ])
  ...
  .factory("TranslationLoader", [
    "TranslationService",
    (TranslationService) => new TranslationLoader(TranslationService),
  ])
