...
export default class EditableMap {
  ...
  humanizeLevel(data) {
    ...
    if (data.reward) {
      data.reward.value = self.$filter("percent")(
        self.$filter("commaToDot")(data.reward.value)
      );
    }
    ...
    if (data.specialRewards) {
      data.specialRewards = _.map(data.specialRewards, function (reward) {
        return {
          ...
          startAt: moment(reward.startAt).format(self.config.dateFormat),
          name: reward.name,
          value: self.$filter("percent")(
            self.$filter("commaToDot")(reward.value)
          ),
        };
      });
    }
  ...
  humanizeEarningRuleFields(data) {
    ...
    if (data.startAt) {
      data.startAt = moment(data.startAt).format(
        self.config.dateTimeForhumanizeEarningRuleFieldsmat
      );
    }
    if (data.endAt) {
    ...
    segment.parts = _.each(segment.parts, (part) => {
      ...
      _.each(part.criteria, (criterium) => {
        ...
        if (criterium.fromDate && criterium.toDate) {
          criterium.fromDate = new moment(criterium.fromDate).format(
            "YYYY-MM-DDTHH:mm:ssZ"
          );
          criterium.toDate = new moment(criterium.toDate).format(
            "YYYY-MM-DDTHH:mm:ssZ"
          );
        }
        let c = {};
    ...
    segment.parts = _.each(segment.parts, (part) => {
      _.each(part.criteria, (criterium) => {
        if (criterium.type === "transaction_percent_in_pos") {
          criterium.percent = self.$filter("percent")(
            self.$filter("commaToDot")(criterium.percent)
          );
        }
        if (criterium.type === "purchase_period") {
          criterium.fromDate = moment(criterium.fromDate).format(
            self.config.dateTimeFormat
          );
          criterium.toDate = moment(criterium.toDate).format(
            self.config.dateTimeFormat
          );
        }
      });
  ...
  humanizeCampaign(data) {
    ...
    if (campaign.campaignActivity) {
      campaign.campaignActivity.activeTo = moment(
        campaign.campaignActivity.activeTo
      ).format(self.config.dateTimeFormat);
      campaign.campaignActivity.activeFrom = moment(
        campaign.campaignActivity.activeFrom
      ).format(self.config.dateTimeFormat);
    }

    if (campaign.campaignVisibility) {
      campaign.campaignVisibility.visibleTo = moment(
        campaign.campaignVisibility.visibleTo
      ).format(self.config.dateTimeFormat);
      campaign.campaignVisibility.visibleFrom = moment(
        campaign.campaignVisibility.visibleFrom
      ).format(self.config.dateTimeFormat);
    }
  ...
  campaign(data) {
    let self = this;
    let campaign = _.omit(angular.copy(data), [
      "name",
      "shortDescription",
      "usageInstruction",
  ...
  convertTranslations(translations) {
    ...
    _.each(translations, (translation) => {
      if (
        !this.availableTranslations.some((t) => t.code === translation.locale)
      ) {
        return;
      }
  ...
  humanizeMaintenance(data) {
    let self = this;
    if (data.bookingDate) {
      data.bookingDate = moment(data.bookingDate).format(
        self.config.dateFormat
      );
      if (data.bookingDate === "Invalid date") {
        data.bookingDate = "";
      }
    }
  ...
  maintenance(data) {
    ...
    // delete maintenance.productSku;
    // delete maintenance.warrantyCenter;
    // delete maintenance.active;
    // delete maintenance.bookingDate;
    // delete maintenance.bookingTime;
    // delete maintenance.createdAt;
    // delete maintenance.customerData.email;
    // delete maintenance.customerData.name;
    // delete maintenance.customerData.phone;
    return maintenance;
  }
}
